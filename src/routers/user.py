...
@user_router.post('/payment-method/add-card', responses={
    status.HTTP_201_CREATED: {"model": Data[MessageResponse], 'description': 'Payment method added'},
    status.HTTP_400_BAD_REQUEST: {"model": Data[MessageResponse], 'description': 'Bad Request'}
}, status_code=status.HTTP_201_CREATED)
def add_payment_method(
        payment_method: PaymentMethodCardRequest,
        current_user: Annotated[BaseUserModel, Depends(get_current_user)],
        mongo_client: Database[Mapping[str, Any]] = Depends(MongoDBClient()),
        stripe_client: StripeClient = Depends(StripeClientInstance())
):
    try:
        ...
        payment_method.userId = ObjectId(current_user.id)

        user_payment_methods_db = mongo_client.payment_method.find(
            {'user_id': ObjectId(current_user.id)})

        user_payment_methods = [pm for pm in user_payment_methods_db]

        if len(user_payment_methods) > 0:
            if payment_method.default:
                exists_default_payment_method = filter(lambda pm: pm.default, user_payment_methods) is not None

                if exists_default_payment_method:
                    raise HttpException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        error_id=ErrorsIDs.USER_ALREADY_HAVE_DEFAULT,
                        description=ErrorsDescriptionsObject[ErrorsIDs.USER_ALREADY_HAVE_DEFAULT].format(
                            'payment method')
                    )
        else:
            payment_method.default = True

        mongo_client.payment_method.insert_one(payment_method.to_schema())

        stripe_client.payment_methods.attach(payment_method.methodInfo.stripeId, params=dict(
            customer=current_user.stripeId
        ))

        return Data[MessageResponse](
            data=MessageResponse(
                message=ResponseDescriptions.RECORD_ADDED_SUCCESS.format('Payment method')
            ).to_json()
        )
...
# @user_router.post('/orders/place', responses={
#     status.HTTP_201_CREATED: {"model": Data[MessageResponse], 'description': 'Order Placed'},
# }, status_code=status.HTTP_201_CREATED)
# def placer_order(
...
#     try:
#         for order in orders:
#             order.userId = ObjectId(current_user.id)
#
#             mongo_client.order.insert_one(order.to_schema())
#
#         return Data[MessageResponse](
#             data=MessageResponse(
#                 message=ResponseDescriptions.ORDER_PLACED_SUCCESS
#             ).to_json()
#         )
#
#     except HttpException as ex:
